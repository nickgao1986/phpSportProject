...
class Upvote extends AuthBase {
    ...
    public function save() {
        ...
        try {
            ...
            if($userNewsId) {
                ...
                return show(config('code.error'), '内部错误 点赞失败12', [], 500);
            }
        }catch (\Exception $e) {
            echo $e->getMessage();die;
            return show(config('code.error'), '内部错误 点赞失败11', [], 500);
        }
    ...
    public function read1() {
        ...
        $userNews = model('UserNews')->getUserNewsByCondition($data, $this->from, $this->size);

        if($userNews) {
            return show(config('code.success'), 'OK', $userNews, 200);
        }else {
            return show(config('code.success'), 'OK', ['isUpvote1' => 0], 200);
        }
